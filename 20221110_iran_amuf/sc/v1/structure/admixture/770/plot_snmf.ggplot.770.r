...
# cd
setwd("/public/home/d0202110815/at42/proj/iran_amuf/structure/snmf/plot")
...
# args
q_f <- '../run/770.6.Q'
pops <- '../sam_ple/snmf.strata'
pop_od_f <- '../sam_ple/pop.order'
op_p <- 'snmf.770.6.idi'
op_p2 <- 'snmf.770.pop'
...
df_n1 <- as.tibble(df) %>%
	...
	map_df(~.x %>% mutate(group=cutree(hclust(dist(as.matrix(.[1:length(df)]))), k=2))) # length(df)
...
# my_pal_2 <- colorRampPalette(my_pal)(13)
# show_col(my_pal_2)
