...
# cd
setwd('/public/home/d0202110815/at42/proj/iran_amuf/hap_net/chr_y/plot')
...
# args
fa_f <- '../fa/sel.fa' # 260; 243
loc_spe <- '../sam_ple/y.loc.hap.ov_oo_oa.strata'
...
# net
hap_net <- haploNet(hap_type) # print.default
...
pop <- l_b$X2 #rep(paste0("pop", 1:4), each = 20)
...
# mypal <- pal_npg("nrc", alpha = 0.7)(length(colnames(P2)))
...
# plot
#getHaploNetOptions
setHaploNetOptions(labels=T, show.mutation = 0, pie.colors.function=mypal_f,
	link.width=0.2, scale.ratio = 1, labels.cex=1, mutations.cex=0.1) # cex = 0.8

## change_steps
hap_net_n <- hap_net
hap_net_n[,3] <- sqrt(hap_net_n[,3]) + 1

# draw
pdf("hapnet.y.loc.lb.pdf", width=7, height=7)
plot(hap_net_n, size=sqrt(sz)*2+2, pie=P2, threshold=0) # sqrt(sz)
...
write_tsv(hap_tsv, "hapnet.y.tsv", col_names = T)

# rds
#write_rds(hap_net, 'mito.hap_net.rds')
