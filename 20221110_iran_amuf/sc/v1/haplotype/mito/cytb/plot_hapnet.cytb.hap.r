...
########### the color of ggsci ###################
length(colnames(P2))
mypal_f <- pal_npg_adaptive("nrc", alpha = 0.7)
mypal <- pal_npg_adaptive("nrc", alpha = 0.7)(length(colnames(P2)))
names(mypal) <- colnames(P2)

########## the color of ggplto2 ##################
# library(scales)
...
setHaploNetOptions(labels=F, show.mutation = 0, pie.colors.function=mypal_f,
	link.width=0.1, scale.ratio = 1) # cex = 0.8

## change_steps
hap_net_n <- hap_net
hap_net_n[,3] <- hap_net_n[,3] + 14

# plot_inter
# plot(hap_net_n, size=sqrt(sz) + 9, pie=P2, threshold=0, fast=F)
# o <- replot()

# draw
pdf("hapnet.cytb.hap.pdf", width=7, height=7)
plot(hap_net_n, size=sqrt(sz), pie=P2, threshold=0, fast=F) # sqrt(sz)
legend(x="topright", legend=names(mypal), bty="n", pt.bg=mypal, col="black", pch=22)
# legend(x="topleft", legend=c("1", "4", "9"), pt.cex=c(7,8,9), bty="n", pt.bg=mypal, col="black", pch=1)
dev.off()
...
write_tsv(hap_tsv, "hapnet.cytb.tsv", col_names = T)
