...
# cd
cd /public/home/d0202110815/at42/proj/iran_amuf/gene/hapnet/fa_pcdh9_exon

# ai_vcf
v_f='/public/home/d0202110815/at42/proj/iran_amuf/chr_auto/miss_maf/ms01_mf001/10.ms01_mf001.vcf.gz'
r_g='pcdh9.exon.rg'

bcftools view $v_f -R $r_g > sel.vcf
...
bgzip sel.sp.vcf
tabix -p vcf sel.sp.vcf.gz

# hap
bcftools query -l sel.sp.vcf.gz > id.ls

ref_fa='/public/home/d0202110815/at42/proj/iran_amuf/genome/chr_auto/sheep_chr_num.fna'
r_g='10:41893112-43073231'
v_f='sel.sp.vcf.gz'
...
snp-sites -v -o hap.vcf hap.fa
#vcf2phylip.py -i hap.vcf -f

# id.strata
grep '>' hap.fa | sed 's/>//' > hap.id
sed 's/\.1$//; s/\.2$//' hap.id > hap.og.id  # this will remove twice
paste hap.og.id hap.id > hap.og_new.id

awk 'BEGIN{FS="\t"; OFS="\t"} NR==FNR{a[$1]=$2; next} ($1 in a){print $0, a[$1]}' sel.id hap.og_new.id > hap.og_new.strata
