#!/usr/bin/env bash
#SBATCH -J sm_sp.at42_id
#SBATCH -p Cnode # Fnode2/Fnode1/Gnode/Cnode
#SBATCH -n 1
#SBATCH -c 1
#SBATCH -t 6-14:00
#SBATCH -o %j.o
#SBATCH -e %j.e

# cd
cd /public/home/d0202110815/at42/proj/iran_amuf/review/ssa/pbs

# args
ch_r='at42_id'

vf_p='/public/home/d0202110815/at42/proj/iran_amuf/review/ssa/d_fd/v_f'

v_f=$vf_p/d.${ch_r}.vcf.gz

p1='sam_ple/ssa.4.tsv'
p2='sam_ple/MEN.tsv'
p3='sam_ple/O.orientalis_2.tsv'
p_all='sam_ple/s.v.id'

# select_vf
bcftools view -S $p_all $v_f > v_f/$ch_r.vcf

# pbscan
pbscan -win 50 -step 25 -mc 2000 -div 2 -vcfp v_f/$ch_r.vcf -pop1 $p1 -pop2 $p2 -pop3 $p3 -out cal/pbs_ssa4.$ch_r

# kk
seq 26 > chr.id
id_ls='chr.id'
m_sh='m.sh'

cat $id_ls | while read at_id
do
    sed "s/at42_id/$at_id/g" $m_sh > sc/$at_id.sh
    chmod 755 sc/$at_id.sh
done

cd sc
for s_h in *sh
do
	printf "%s:\t" $s_h
    sbatch --parsable $s_h
done > ../sbatch.log
